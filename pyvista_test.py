...
def main(argv):
    ...
    )  # a function that creates a ramp
    opacity_function.AddPoint(
        smin, 0.0
    )  # add control points on that color ramp
    ...
    colour_function.AddRGBPoint(
        smin, 0.0, 0.0, 0.0
    )  # can do RGB space with minimum mapping
    colour_function.AddRGBPoint(
        smax, 1.0, 1.0, 1.0
    )  # maximum scalar value mapped

    volume_property = (
        vtk.vtkVolumeProperty()
    )  # connects the volume transfer function to color mapping
    volume_property.SetColor(colour_function)
    volume_property.SetScalarOpacity(opacity_function)
    volume_property.ShadeOn()
    # adds shading, but will introduce artefacts (and CPU/GPU load)
    volume_property.SetInterpolationTypeToLinear()
    volume_mapper = vtk.vtkSmartVolumeMapper()
    # breakpoint()
    volume_mapper.SetInputData(
        mesh
    )  # error: The SetInput method of this mapper requires either a vtkImageData or a vtkRectilinearGrid as input
